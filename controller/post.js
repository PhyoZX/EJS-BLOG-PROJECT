...
exports.renderCreatePage = (req,res,next) =>{
    res.render("addpost",{title: "Create Post",errorMsg : "" , oldFormData : { title: "",photo: "",description:"" }});
};
...
exports.renderHomePage = (req,res,next) =>{
    ...
    Post.find().countDocuments().then(totalPostCount => {
        ...
        if(posts.length > 0){
            ...
            res.status(500).render("error/500", { title : "Something went wrong", message : "No post found at this page."})
        }
    }).catch(err => {
        console.log(err);
        const error = new Error("Something Went Wrong")
...
exports.getPost = (req,res,next) =>{
    ...
    .then((post)=>{
        res.render("details",{
                ...
                currrentLoginUserStatus : req.user ? req.user.isPremium : "",
            })
    })
    .catch((err)=>{
        console.log(err);
...
exports.getEditPost = (req,res,next) =>{
    ...
    .then(post=>{
        ...
        res.render("editpost",{postId,title: post.title,post,errorMsg:"",oldFormData: { title: undefined,description: undefined},isValidationFail: false});
    })
    .catch(err => {
        console.log(err);
        const error = new Error("Something Went Wrong")
...
exports.deletePost = (req,res,next)=>{
    ...
    Post
        ...
        .then(post=>{
            ...
            fileDelete(post.imgUrl);
    return Post
        .deleteOne({ _id: postId, userId: req.user._id})
        .then(()=>{
